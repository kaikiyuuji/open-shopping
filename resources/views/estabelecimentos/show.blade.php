@extends('layouts.main')

@section('title', $estabelecimento->nome)

@section('content')
    <div class="mb-8 flex items-end justify-between">
        <div>
            <p class="mb-1 text-xs uppercase tracking-widest text-neutral-500">{{ $estabelecimento->categoria }}</p>
            <h1 class="text-3xl font-light tracking-tight">{{ $estabelecimento->nome }}</h1>
            <p class="mt-1 text-sm text-neutral-600">{{ $estabelecimento->endereco }}</p>
        </div>
        <div class="flex gap-3 text-xs uppercase tracking-widest">
            <a href="{{ route('estabelecimentos.edit', $estabelecimento) }}"
               class="border border-black px-4 py-2 transition hover:bg-neutral-100">Editar</a>
            <a href="{{ route('estabelecimentos.index') }}"
               class="border border-black px-4 py-2 transition hover:bg-neutral-100">Voltar</a>
        </div>
    </div>

    <h2 class="mb-4 border-b-2 border-black pb-2 text-sm uppercase tracking-widest">Compras neste estabelecimento</h2>

    @if ($estabelecimento->compras->isEmpty())
        <p class="border border-dashed border-neutral-300 px-6 py-12 text-center text-sm text-neutral-500">
            Nenhuma compra registrada.
        </p>
    @else
        <table class="w-full border-collapse text-sm">
            <thead>
                <tr class="border-b border-neutral-300 text-left uppercase tracking-widest">
                    <th class="py-3 pr-4 font-medium">Data</th>
                    <th class="py-3 pr-4 font-medium">Valor</th>
                    <th class="py-3 pr-4 font-medium">Pagamento</th>
                    <th class="py-3 text-right font-medium">Ações</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($estabelecimento->compras as $compra)
                    <tr class="border-b border-neutral-200 transition hover:bg-neutral-50">
                        <td class="py-3 pr-4">{{ $compra->data->format('d/m/Y') }}</td>
                        <td class="py-3 pr-4">R$ {{ number_format($compra->valor_total, 2, ',', '.') }}</td>
                        <td class="py-3 pr-4 capitalize">
                            {{ $compra->forma_pagamento }}
                            @if ($compra->parcelado)
                                · {{ $compra->numero_parcelas }}x
                            @endif
                        </td>
                        <td class="py-3 text-right text-xs uppercase tracking-widest">
                            <a href="{{ route('compras.show', $compra) }}" class="underline-offset-4 hover:underline">Detalhes</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    @endif
@endsection
